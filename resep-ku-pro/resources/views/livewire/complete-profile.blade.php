    ...
    <div class="max-w-6xl w-full relative z-10"
        ...
        <div class="grid md:grid-cols-3 gap-6">
            ...
            <div @click="role = 'manager'"
                 ...
                 class="bg-white p-8 rounded-[2.5rem] shadow-xl cursor-pointer transition-all border-4 text-center group">
                <div :class="role === 'manager' ? 'bg-[#f97316]' : 'bg-orange-50'" class="w-16 h-16 rounded-2xl flex items-center justify-center mx-auto mb-4">
        ...
        <div x-show="role" x-transition class="mt-8 bg-white p-8 rounded-[3rem] shadow-2xl max-w-xl mx-auto space-y-6">
            ...
            <div>
                ...
                <input type="text" wire:model="full_name" class="w-full px-5 py-3 bg-gray-50 border-2 rounded-xl focus:border-[#f97316] outline-none font-bold">
            </div>
            ...
            <div x-show="role === 'owner'" class="space-y-4">
                <div>
                    ...
                    <input type="text" wire:model.live="brand_name" @input.debounce.500ms="generateOrgId()" class="w-full px-5 py-3 bg-gray-50 border-2 rounded-xl focus:border-[#f97316] outline-none font-bold">
                </div>
                <div>
                    <label class="text-xs font-black uppercase tracking-widest text-gray-400">Generated Org ID</label>
                    <input type="text" wire:model="team_org_id" readonly class="w-full px-5 py-3 bg-gray-100 border-2 rounded-xl font-mono font-bold text-[#f97316]">
                </div>
            </div>
            ...
            <div x-show="role === 'manager' || role === 'staff'" class="space-y-4">
                ...
                <input type="text" wire:model="team_org_id" class="w-full px-5 py-3 bg-gray-50 border-2 rounded-xl focus:border-[#f97316] outline-none font-bold" placeholder="ID from your Owner">
            </div>

            <button wire:click="saveProfile" class="w-full bg-[#f97316] text-white font-black py-4 rounded-xl shadow-lg hover:bg-orange-600 transition-all">
                {{ __('START COOKING!') }}
            </button>
        </div>
    </div>
