    ...
    <div class="max-w-4xl mx-auto space-y-6">
        ...
        <div class="bg-white rounded-[2rem] p-8 shadow-sm border border-gray-100">
            ...
            <form wire:submit="save" class="flex flex-col md:flex-row gap-4">
                <div class="flex-1">
                    <label class="text-[10px] font-black uppercase text-gray-400 mb-2 block tracking-widest">Category
                        Name</label>
                ...
                <div class="flex gap-2 items-end">
                    <button type="submit"
                        class="bg-[#1a1a1a] hover:bg-black text-white px-10 py-4 rounded-2xl font-black text-xs uppercase tracking-widest transition-all cursor-pointer shadow-lg active:scale-95">
                        {{ $is_editing ? 'Update' : 'Save' }}
                    </button>
                    @if ($is_editing)
                        <button type="button" wire:click="resetForm"
                            class="bg-gray-100 text-gray-500 px-6 py-4 rounded-2xl font-black text-xs uppercase cursor-pointer hover:bg-gray-200">Cancel</button>
                    @endif
                </div>
            </form>
        </div>
